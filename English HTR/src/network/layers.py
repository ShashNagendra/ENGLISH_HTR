...
class FullGatedConv2D(Conv2D):
    ...
    def call(self, inputs):
        ...
        sigmoid_spatial = Activation("sigmoid")(output[:, :, :, self.nb_filters:(self.nb_filters*2)])
        sigmoid_channel = Activation("sigmoid")(output[:, :, :, (self.nb_filters*2):])

        return Multiply()([Multiply()([linear, sigmoid_spatial]), sigmoid_channel])
